...
public class TaskService {
     ...
     * Log.debug("tel:"+tel+" 查看任务状态，任务编号："+num); if (customer != null) { try { num = URLDecoder.decode(num, "UTF-8");
     * System.out.println("num:"+num); Task task = taskDao.getTaskByNum(num); if (task == null) {
     * Log.error("tel:"+tel+" 查看任务状态，任务编号："+num+"--任务检索不存在"); return Message.ERROR; } TaskPerson person =
     ...
     * StringBuffer(); sb.append("您查询的事项编号为"); sb.append(num); if (task.getInsertTime() != null) { sb.append(",提出时间为");
     * sb.append(dateParse.date2AnotherString(task.getInsertTime())); } sb.append(","); sb.append(task.getRemark());
     * sb.append(";目前进度为:"); sb.append(StatusParser.getStatus(task.getStatus().trim())); if (employee != null) {
     * sb.append(",责任人为"); sb.append(employee.getName()); }else {
     * Log.error("tel:"+tel+" 查看任务状态，任务编号："+num+"，责任人查找失败：task num"+task.getNum());
     * System.out.println("责任人查找失败：task num"+task.getNum()); } sb.append("。您还可以打电话给奚女士（13816156445）了解详情.");
     * Log.debug("tel:"+tel+" 查看任务状态，任务编号："+num+"查看成功--result:"+sb.toString()); return sb.toString(); } catch (Exception
     * e) { // TODO: handle exception Log.error("tel:"+tel+" 查看任务状态，任务编号："+num+"--数据解析错误"); e.printStackTrace(); } }
     * Log.error("tel:"+tel+" 查看任务状态，任务编号："+num+"--身份验证失败"); return Message.ERROR; }
     */
	...
	public String getProgress(@HeaderParam("tel") String tel,@HeaderParam("num") String num) {
		...
		if (customer != null) {
			try {
                ...
                Log.debug("tel:" + tel + " 查看任务状态，任务编号：" + num + "查看成功--result:" + res);
				return res;
			} catch (Exception e) {
